...
def _cf_get(url: str, headers: dict = None, timeout: int = 15) -> bytes:
    """Try curl_cffi first (Cloudflare bypass), fall back to urllib."""
    h = headers or HEADERS
    try:
        from curl_cffi import requests as cr
        resp = cr.get(url, headers={k: v for k, v in h.items() if k != "User-Agent"},
...
def _http_get(url: str, headers: dict = None, timeout: int = 15) -> bytes:
    return _cf_get(url, headers, timeout)


def _get_json(url: str, headers: dict = None) -> Optional[dict]:
    try:
        data = _cf_get(url, headers)
